...
export const CategoryProductsClient: React.FC<CategoryProductsClientProps> = ({
    ...
    return (
      <div className="bg-white rounded-2xl p-8 border border-slate-200 text-center space-y-4 shadow-xs">
        ...
        <div className="space-y-1">
          <h3 className="text-sm font-semibold text-slate-900">Category Feed Updating</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            Live prices for {categoryName} in {market.toUpperCase()} are currently refreshing.
          </p>
        </div>
        <button
  ...
  return (
    <EmptyState
      title={`No ${categoryName} currently indexed in ${market.toUpperCase()}.`}
